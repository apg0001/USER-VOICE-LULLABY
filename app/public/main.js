...
document
  ...
  .addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (form.reverb_room_size && form.reverb_room_size.value.trim().length > 0) {
      ...
      if (form.reverb_wet_gain && form.reverb_wet_gain.value.trim().length > 0) {
        formData.append("reverb_wet_gain", form.reverb_wet_gain.value);
      }
      if (form.reverb_dry_gain && form.reverb_dry_gain.value.trim().length > 0) {
      ...
      if (form.reverb_freeze_mode && form.reverb_freeze_mode.value.trim().length > 0) {
