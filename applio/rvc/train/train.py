...
# Zluda hijack
import applio.rvc.lib.zluda
from applio.rvc.lib.algorithm import commons
from applio.rvc.train.process.extract_model import extract_model
...
experiment_dir = os.path.join(current_dir, "applio", "logs", model_name)
...
def main():
    ...
    if cleanup:
        ...
        for root, dirs, files in os.walk(
            os.path.join(now_dir, "applio", "logs", model_name), topdown=False
        ):
            for name in files:
...
def run(
    ...
    from applio.rvc.lib.algorithm.discriminators import MultiPeriodDiscriminator
    ...
    if os.path.isfile(os.path.join("applio", "logs", "reference", embedder_name, "feats.npy")):
        ...
        pitch = np.load(os.path.join("applio", "logs", "reference", "pitch_coarse.npy"))
        # removed last frame to match features
        pitch = torch.LongTensor(pitch[:-1]).unsqueeze(0).to(device)
        pitchf = np.load(os.path.join("applio", "logs", "reference", "pitch_fine.npy"))
        # removed last frame to match features
