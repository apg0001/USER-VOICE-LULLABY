...
from applio.rvc.lib.predictors.RMVPE import RMVPE0Predictor
...
class RMVPE:
    def __init__(self, device, model_name="rmvpe.pt", sample_rate=16000, hop_size=160):
        ...
        self.model = RMVPE0Predictor(
            os.path.join("applio", "rvc", "models", "predictors", model_name),
            device=self.device,
        )
...
class FCPE:
    def __init__(self, device, sample_rate=16000, hop_size=160):
        ...
        self.model = spawn_infer_model_from_pt(
            os.path.join("applio", "rvc", "models", "predictors", "fcpe.pt"),
            self.device,
            bundled_model=True,
        )
